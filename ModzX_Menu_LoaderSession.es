...
var autoArmorOn     = false;
var autoLeaveOn     = false;
var autoSpawnArmed  = true;
var autoKillArmed   = true;
var autoLeaveArmed  = true;
var autoArmorTick   = 0;
...
// TP visual controlado: pasos finitos para dar tiempo a cargar chunks sin repetir el TP indefinidamente.
var visualTp = {
    active: false,
    targetX: 0,
    targetY: 0,
...
//  SELECTOR - FUNCIONES
// ============================================================
function getEntityTypeCompat(ent) {
    var type = null;
    try {
        if (typeof Entity.getEntityTypeId == "function") type = Entity.getEntityTypeId(ent);
    } catch(e) {}
    if (type === null) {
        try {
            if (typeof nativeGetEntityTypeId == "function") type = nativeGetEntityTypeId(ent);
        } catch(e) {}
    }
    if (type === null) {
        try {
            if (typeof Entity.getType == "function") type = Entity.getType(ent);
        } catch(e) {}
    }
    return type;
}

function isRealPlayerEntity(ent) {
    if (ent == myEnt()) return false;

    // En MCPE/ModPE, el tipo 63 corresponde al jugador real.
    var type = getEntityTypeCompat(ent);
    if (type !== null && type !== undefined && type !== "") {
        var numericType = Number(type);
        if (!isNaN(numericType) && numericType > 0) return numericType == 63;
    }

    // Compatibilidad si la build no expone la API de tipo de entidad.
    var pname = "";
    try { pname = String(Player.getName(ent) || ""); } catch(e) {}
    if (pname === "") return false;
    if (pname.indexOf("§8") === 0) return false;

    var lowerName = pname.toLowerCase();
    if (lowerName.indexOf("npc") !== -1 || lowerName.indexOf("slapper") !== -1 ||
        lowerName.indexOf("shopkeeper") !== -1 || lowerName.indexOf("citizen") !== -1) return false;
    return true;
}
...
function filterPlayers() {
    ...
    try {
        ...
        for (var i = 0; i < players.length; i++) {
            if (!isRealPlayerEntity(players[i])) continue;
            var pname = "";
            try { pname = String(Player.getName(players[i]) || ""); } catch(e) {}
            if (pname === "") continue;
...
function getStatusLine() {
    ...
    if (autoArmorOn)    parts.push("AutoArmor");
    if (autoLeaveOn)    parts.push("AutoLeave");
...
function refreshPlayerSnapshot() {
    ...
    try {
        var players = Server.getAllPlayers();
        for (var i = 0; i < players.length; i++) {
            if (isRealPlayerEntity(players[i])) {
    ...
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            makeActionRow(inner, "[X]", "Desactivar todo", "Apaga todos los modulos", true, function() {
                ...
                autoKillOn = false; autoArmorOn = false; autoLeaveOn = false;
                autoSpawnArmed = true; autoKillArmed = true; autoLeaveArmed = true; autoArmorTick = 0;
...
function showPlayerActions(playerEnt, playerName) {
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            makeActionRow(inner, "[TP]", "TP visual a " + playerName, "Carga el destino por pasos y se detiene si el servidor corrige", false, function() {
                startVisualTp(playerEnt, playerName);
            });
...
function menuMovimiento() {
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            makeToggleRow(inner, "Auto-Armor", "Busca casco, pechera, pantalones y botas en el inventario.",
                autoArmorOn, function(v) {
                    autoArmorOn = v;
                    autoArmorTick = 0;
                    clientMessage(v ? "§dAuto-Armor ON" : "§7Auto-Armor OFF");
                }, C.SUCCESS
            );
            makeToggleRow(inner, "Auto Leave Server", "Sale al instante al bajar de 4 corazones.",
                autoLeaveOn, function(v) {
                    autoLeaveOn = v;
                    autoLeaveArmed = true;
                    clientMessage(v ? "§dAuto Leave Server ON" : "§7Auto Leave Server OFF");
                }, C.DANGER
            );
...
function armorSlotForItem(itemId) {
    var ids = [
        [298, 302, 306, 310, 314], // casco
        [299, 303, 307, 311, 315], // pechera
        [300, 304, 308, 312, 316], // pantalones
        [301, 305, 309, 313, 317]  // botas
    ];
    for (var slot = 0; slot < ids.length; slot++) {
        for (var j = 0; j < ids[slot].length; j++) {
            if (Number(itemId) == ids[slot][j]) return slot;
        }
    }
    return -1;
}

function armorTier(itemId) {
    itemId = Number(itemId);
    if (itemId >= 310 && itemId <= 313) return 4; // diamante
    if (itemId >= 314 && itemId <= 317) return 3; // oro
    if (itemId >= 306 && itemId <= 309) return 2; // hierro
    if (itemId >= 302 && itemId <= 305) return 1; // cota de malla
    if (itemId >= 298 && itemId <= 301) return 0; // cuero
    return -1;
}

function equipMissingArmor() {
    var equipped = 0;
    try {
        for (var slot = 0; slot < 4; slot++) {
            var currentId = 0;
            try { currentId = Number(Player.getArmorSlot(slot)); } catch(e) {}
            if (currentId > 0) continue;

            var bestId = -1;
            var bestDamage = 0;
            var bestTier = -1;
            for (var inv = 0; inv < 36; inv++) {
                var itemId = -1;
                var count = 1;
                try { itemId = Number(Player.getInventorySlot(inv)); } catch(e) {}
                try { count = Number(Player.getInventorySlotCount(inv)); } catch(e) {}
                if (itemId <= 0 || count <= 0 || armorSlotForItem(itemId) != slot) continue;

                var tier = armorTier(itemId);
                if (tier > bestTier) {
                    bestId = itemId;
                    bestTier = tier;
                    try { bestDamage = Number(Player.getInventorySlotData(inv)); } catch(e) { bestDamage = 0; }
                }
            }

            if (bestId > 0) {
                try {
                    Player.setArmorSlot(slot, bestId, bestDamage);
                    equipped++;
                } catch(e) {}
            }
        }
    } catch(e) {}
    if (equipped > 0) clientMessage("§aAuto-Armor: " + equipped + " pieza(s) equipada(s)");
}

function startVisualTp(targetEnt, targetName) {
    try {
        var sx = Number(myX()), sy = Number(myY()), sz = Number(myZ());
        var tx = Number(Entity.getX(targetEnt)) + 1;
        var ty = Number(Entity.getY(targetEnt)) + 1;
        var tz = Number(Entity.getZ(targetEnt)) + 1;
        var dx = tx - sx, dy = ty - sy, dz = tz - sz;
        var distance = Math.sqrt(dx * dx + dy * dy + dz * dz);

        if (!isFinite(distance) || distance < 0.5) {
            clientMessage("§eTP visual: ya estas cerca del destino");
            return;
        }

        // Limita el recorrido para evitar teletransportes extremos y bucles largos.
        if (distance > 512) {
            var ratio = 512 / distance;
            tx = sx + dx * ratio;
            ty = sy + dy * ratio;
            tz = sz + dz * ratio;
            distance = 512;
        ...
        visualTp.active = true;
        visualTp.targetX = tx;
        visualTp.targetY = ty;
        visualTp.targetZ = tz;
        visualTp.remaining = Math.ceil(distance / 8) + 2;
        visualTp.wait = 0;
        visualTp.stagnant = 0;
        visualTp.lastDistance = distance;
        clientMessage("§dTP visual iniciado a " + targetName + " · cargando chunks...");
    } catch (e) {
        visualTp.active = false;
        clientMessage("§cTP visual error: " + e);
    }
}

function processVisualTp(me) {
    if (!visualTp.active) return;
    ...
    try {
        ...
        var cx = Number(Entity.getX(me));
        var cy = Number(Entity.getY(me));
        var cz = Number(Entity.getZ(me));
        var dx = visualTp.targetX - cx;
        var dy = visualTp.targetY - cy;
        var dz = visualTp.targetZ - cz;
        var distance = Math.sqrt(dx * dx + dy * dy + dz * dz);

        if (!isFinite(distance) || distance <= 3 || visualTp.remaining <= 0) {
            if (distance <= 3) {
                Entity.setPosition(me, visualTp.targetX, visualTp.targetY, visualTp.targetZ);
            }
            visualTp.active = false;
            clientMessage("§aTP visual finalizado · chunks solicitados");
            return;
        }

        if (visualTp.lastDistance !== null && distance > visualTp.lastDistance + 2) {
            visualTp.active = false;
            clientMessage("§eTP visual detenido: el servidor corrigio la posicion");
            return;
        }

        if (visualTp.lastDistance !== null && Math.abs(distance - visualTp.lastDistance) < 0.15) {
            visualTp.stagnant++;
        } else {
            visualTp.stagnant = 0;
        }
        if (visualTp.stagnant >= 3) {
            visualTp.active = false;
            clientMessage("§eTP visual detenido: no se pudo avanzar");
            return;
        }

        var step = Math.min(8, distance);
        Entity.setPosition(me,
            cx + dx / distance * step,
            cy + dy / distance * step,
            cz + dz / distance * step
        );
        visualTp.remaining--;
        visualTp.wait = 2;
        visualTp.lastDistance = distance - step;
    } catch (e) {
        visualTp.active = false;
        clientMessage("§cTP visual detenido: " + e);
    }
}
...
function modTick() {
    ...
    // TP visual: una sola progresion finita, sin loop de teletransporte continuo.
    processVisualTp(me);
    ...
    // AUTO LEAVE / AUTO SPAWN / AUTO KILL
    try {
        ...
        if (autoLeaveOn && ownHp < 8 && autoLeaveArmed) {
            autoLeaveArmed = false;
            clientMessage("§cAuto Leave Server: saliendo...");
            ModPE.leaveGame();
            return;
        }
    ...
    // Limpieza de objetivos que hayan dejado de ser jugadores reales.
    if (selectedPlayer != null && !isRealPlayerEntity(selectedPlayer)) {
        selectedPlayer = null;
        selectedPlayerName = "";
        tipDistOn = false;
        tipCoordsOn = false;
    }
    if (distFloatTarget != null && !isRealPlayerEntity(distFloatTarget)) {
        distFloatOn = false;
        distFloatTarget = null;
        clearDistFloat();
    }
    ...
    if (killAuraOn) {
        try {
            ...
            for (var i = 0; i < players2.length; i++) {
                if (!isRealPlayerEntity(players2[i])) continue;
        ...
        if (autoAttackTick <= 0) {
            ...
            try {
                ...
                for (var i = 0; i < players3.length; i++) {
                    if (!isRealPlayerEntity(players3[i])) continue;
...
ModPE.langEdit("menu.copyright", "§dModz X §fV52");
