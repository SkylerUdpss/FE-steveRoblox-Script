...
function makeBackBtn(layout, popRef, targetFn) {
    ...
    btn.setOnClickListener(new View.OnClickListener({
        onClick: function() {
            if (popRef[0] && popRef[0].isShowing()) popRef[0].dismiss();
            targetFn();
        }
    }));
    layout.addView(btn);
    ...
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            fab.setOnClickListener(new View.OnClickListener({
                onClick: function() { mainMenu(); GUI.dismiss(); }
            }));
            root.addView(fab);

            GUI = new PopupWindow(root, dip(52), dip(52));
            GUI.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
            GUI.showAtLocation(ctx.getWindow().getDecorView(), Gravity.RIGHT | Gravity.TOP, dip(4), dip(28));
        } catch(e) { print("FAB: " + e); }
    }}));
}
    ...
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            makeActionRow(inner, "[X]", "Desactivar todo", "Apaga todos los modulos", true, function() {
                ...
                findChestOn = false; findHopperOn = false; distFloatOn = false;
                ...
                stealthKillOn = false; hitboxOn = false; hitboxSize = 2.0;
            ...
            closeBtn.setOnClickListener(new View.OnClickListener({
                onClick: function() { menu.dismiss(); menuBtn(); }
            }));
...
function menuCombate() {
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            makeSliderRow(inner, "Hitbox Size", "Tamanio de hitbox (1.0 = normal, 4.0 = maximo)",
                hitboxSize, 1.0, 4.0, 0.5, function(val) {
                    hitboxSize = val;
            ...
            var popRef = [null];
            makeBackBtn(inner, popRef, function() { popRef[0].dismiss(); mainMenu(); });
...
function showPlayerActions(playerEnt, playerName) {
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            var popRef = [null];
            makeBackBtn(inner, popRef, function() { popRef[0].dismiss(); menuPlayer(); });
...
function menuMovimiento() {
    ctx.runOnUiThread(new Runnable({ run: function() {
        try {
            ...
            makeToggleRow(inner, "God Mode", "Regen permanente + Resistencia.",
                godModeOn, function(v) {
                    godModeOn = v;
                    if (v) {
                        Entity.addEffect(myEnt(), 10, 99999, 5, false, true);
                        Entity.addEffect(myEnt(), 11, 99999, 4, false, true);
                        clientMessage("§dGod Mode ON");
                    } else {
                        try { Entity.removeEffect(myEnt(), 10); } catch(e){}
                        try { Entity.removeEffect(myEnt(), 11); } catch(e){}
                        clientMessage("§7God Mode OFF");
                    }
                }, C.DANGER
            );
